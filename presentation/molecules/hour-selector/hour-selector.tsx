...
export function HourSelector({ onSelectionChange, onBack }: HourSelectorProps) {
    ...
    const [displayTimes, setDisplayTimes] = useState<Array<{ display: string, original: string }>>(
    ...
    useEffect(() => {
        if (selection.timezone) {
            ...
            if (offsetMatch) {
                ...
                // Offset base (la aplicación usa UTC-5 como base)
                const baseOffset = -4;
                ...
                setDisplayTimes(schedules.map(time => ({ display: time, original: time })));
            }
        }
    }, [selection.timezone]);
    ...
    return (
        <Card className="bg-white w-72 h-80 border-[#E4E4E7]">
            <CardHeader className="items-start text-center p-0">
                <Button className="bg-white w-3 h-6 mt-4 ml-2" onClick={onBack}>
                    <ChevronLeft strokeWidth={3} />
                </Button>
            </CardHeader>
            <CardContent className="flex flex-col text-center items-center">
                <CardTitle className="font-medium text-base text-secondary mb-5">
                    Select an hour for the call
                </CardTitle>
                <div>
                    <Select
                        onValueChange={(value) =>
                    ...
                    >
                        ...
                        <SelectContent>
                            <SelectGroup>
                                ...
                                <SelectItem value="UTC-09:00">(UTC-09:00) Anchorage, Juneau</SelectItem>
                                <SelectItem value="UTC-08:00">(UTC-08:00) Los Angeles, Vancouver</SelectItem>
                                <SelectItem value="UTC-07:00">(UTC-07:00) Phoenix, Las vegas</SelectItem>
                                <SelectItem value="UTC-06:00">(UTC-06:00) Denver</SelectItem>
                                <SelectItem value="UTC-06:00">(UTC-06:00) Mexico City, Guatemala City</SelectItem>
                                <SelectItem value="UTC-05:00">(UTC-05:00) New Orleans, New york</SelectItem>
                                <SelectItem value="UTC-05:00">(UTC-05:00) Bogotá,Quito,Lima</SelectItem>
                                <SelectItem value="UTC-04:00">(UTC-04:00) Caracas, Santo Domingo</SelectItem>
                                <SelectItem value="UTC-03:00">(UTC-03:00) Buenos Aires, Montevideo</SelectItem>
                            </SelectGroup>
                        </SelectContent>
                    </Select>
                </div>
                        ...
                        <div className="flex flex-col space-y-2 p-4">
                            {displayTimes.map((timeObj) => (
                                <Button
                                    ...
                                    variant={selection.hour === timeObj.original ? "default" : "outline"}
                                    onClick={() =>
                                        setSelection((prev) => ({ ...prev, hour: timeObj.original }))
                                    }
